  ...
  try {
    ...
    try {
      ...
      if (payload.action === 'test_connection') {
        ...
        // Connect to Supabase
        try {
          const response = await fetch(`${supabaseUrl}/rest/v1/${payload.table || 'userfile'}?${payload.query || 'select=count'}`, {
            method: 'GET',
            headers: {
              'apikey': supabaseKey,
      ...
      if (payload.action === 'authenticate') {
        ...
        try {
          ...
          const tables = ['userfile', 'users', 'profiles'];
          let userData = null;
          let tableUsed = null;
      ...
      if (payload.action === 'get_user') {
        ...
        try {
          ...
          // Get additional user data from userfile table if needed
          let appUserData = null;
          try {
            const userResponse = await fetch(`${supabaseUrl}/rest/v1/userfile?id=eq.${userData.id}&select=*`, {
              method: 'GET',
              headers: {
                'apikey': supabaseKey,
                'Authorization': `Bearer ${supabaseKey}`
            ...
            console.error('Error getting userfile data:', e);
          }
          
          // Merge auth user data with userfile data if available
          const user = {
            ...userData,
            ...(appUserData || {})
          };
