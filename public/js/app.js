...
async function fetchUserProfile() {
    ...
    try {
        const response = await fetch('/api/profile', {
            ...
            body: JSON.stringify({ 
                username: username,
                createIfNotExists: true // Enable auto-creation of profile if not found
            })
        });
        ...
        if (response.ok) {
            ...
            if (data.success && data.data) {
                ...
                // If this is a newly created user, show a notification
                if (data.isNewUser && typeof M !== 'undefined' && M.toast) {
                    M.toast({
                        html: 'Welcome! Your profile has been created automatically.',
                        classes: 'green',
                        displayLength: 4000
                    });
                }
